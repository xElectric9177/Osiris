...
Item {
  ...
  // Line the card up with amendale.bar's left island: same left edge, dropped
  // clear of the bar by the same gap the islands use between themselves.
  readonly property int barGap: Style.space(8)
  readonly property int barInset: barGap
  readonly property int barDrop: Style.bar.sizeHorizontal + barGap
  readonly property int slideDistance: Style.space(12)
  ...
  property int cardWidth: Math.min(root.dmenuActive ? Style.space(root.dmenuWidth) : ((root.activeMenu === "trigger.capture.screenrecord" || root.activeMenu === "style.font") ? Style.space(520) : Style.space(300)), panel.width - root.barInset - Style.gapsOut)
  ...
  // Height the card can devote to rows before running off the bottom of the
  // screen, measured down from its fixed top edge under the bar.
  function availableRowsHeight() {
    var available = panel.height - panel.cardTop - Style.gapsOut - root.contentMargin * 2 - root.headerHeight - root.contentSpacing
  ...
  PanelWindow {
    ...
    // The card hangs off the bar's own launcher island rather than floating
    // centered: same left edge as the left island, dropped just below the
    // bar. A fixed top also means the card can only ever grow downward, so
    // the jump-on-resize the centered layout had can't happen at all.
    // maxRowsHeight is still frozen on the first keystroke/submenu move, so
    // the starting menu caps how tall the card may grow from there.
    property int maxRowsHeight: -1
    readonly property int cardTop: root.barDrop
    readonly property int effectiveCardTop: cardTop
    function freezeCardTop() {
      if (visible && maxRowsHeight < 0) maxRowsHeight = root.visibleRowsHeight
    }
    onVisibleChanged: if (!visible) maxRowsHeight = -1
    ...
    BorderSurface {
      ...
      x: root.barInset
      // Slides down out of the bar on open, matching the system popups.
      // Driven off panel.visible rather than root.opened: `opened` flips
      // before rowsLoaded, so animating on it would burn part of the travel
      // while the window is still unmapped.
      y: panel.effectiveCardTop + (panel.visible ? 0 : -root.slideDistance)
      color: root.background
      borderSpec: root.borderSpec
      padding: root.contentMargin

      // Closing hides the window outright, so only the open direction is
      // ever seen — no retraction easing needed here.
      Behavior on y {
        NumberAnimation {
          duration: 600
