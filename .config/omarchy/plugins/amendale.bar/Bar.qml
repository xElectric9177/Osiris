...
Item {
  ...
  // Not `required`: the host's pluginBarLoader instantiates this file via
  // Loader.source, which creates the root item before any property can be
  // supplied, so a `required property` here fails construction outright.
  // shell.qml's configureBar() sets this from onLoaded once the item exists.
  ...
  component BarPanel: PanelWindow {
    ...
    Component {
      ...
      Item {
        ...
        readonly property int islandHeight: Math.max(root.barSize - Style.space(4), 1)
        readonly property int islandGap: Style.space(8)
        readonly property color islandGlow: Qt.rgba(Color.accent.r, Color.accent.g, Color.accent.b, 0.8)
        // The tray reserves layout space for its fully-expanded drawer at all
        // times (implicitWidth never shrinks), so other bar content doesn't
        // jump when it opens. drawerExtent/revealExtent are its own public,
        // already-animated account of how much of that reservation is
        // currently unused — subtract it so the pill fits the tray's real
        // visual width and only grows when the drawer is actually expanded.
        readonly property var rightTrayWidget: {
          if (!rightGroup.item) return null
          var kids = rightGroup.item.children
          for (var i = 0; i < kids.length; i++) {
            if (kids[i] && kids[i].moduleName === "omarchy.tray") return kids[i].activeItem
          }
          return null
        }
        readonly property real rightTrayUnusedReserve: (rightTrayWidget && "drawerExtent" in rightTrayWidget && "revealExtent" in rightTrayWidget)
          ? Math.max(0, rightTrayWidget.drawerExtent - rightTrayWidget.revealExtent)
          : 0
        ...
        BorderSurface {
          ...
          width: leftGroup.width + Style.space(16)
        ...
        BorderSurface {
          ...
          anchors.verticalCenter: parent.verticalCenter
          width: rightGroup.width - parent.rightTrayUnusedReserve + Style.space(16)
          height: parent.islandHeight
          radius: height / 2
          color: "transparent"
          borderSpec: Border.flat(parent.islandGlow, 1)
    ...
    Component {
      ...
      Item {
        ...
        readonly property int islandWidth: Math.max(root.barSize - Style.space(4), 1)
        readonly property int islandGap: Style.space(8)
        readonly property color islandGlow: Qt.rgba(Color.accent.r, Color.accent.g, Color.accent.b, 0.8)
        // Mirrors rightTrayUnusedReserve in horizontalBar: the tray reserves
        // room for its fully-expanded drawer at all times, so shrink the pill
        // by whatever part of that reservation isn't currently revealed.
        readonly property var bottomTrayWidget: {
          if (!bottomGroup.item) return null
          var kids = bottomGroup.item.children
          for (var i = 0; i < kids.length; i++) {
            if (kids[i] && kids[i].moduleName === "omarchy.tray") return kids[i].activeItem
          }
          return null
        }
        readonly property real bottomTrayUnusedReserve: (bottomTrayWidget && "drawerExtent" in bottomTrayWidget && "revealExtent" in bottomTrayWidget)
          ? Math.max(0, bottomTrayWidget.drawerExtent - bottomTrayWidget.revealExtent)
          : 0
        ...
        BorderSurface {
          ...
          height: topGroup.height + Style.space(16)
          radius: width / 2
          color: "transparent"
          borderSpec: Border.flat(parent.islandGlow, 1)
        }
        ...
        BorderSurface {
          visible: bottomGroup.height > 0
          anchors.bottom: parent.bottom
          anchors.bottomMargin: parent.islandGap
          anchors.horizontalCenter: parent.horizontalCenter
          width: parent.islandWidth
          height: bottomGroup.height - parent.bottomTrayUnusedReserve + Style.space(16)
          radius: width / 2
          color: "transparent"
          borderSpec: Border.flat(parent.islandGlow, 1)
        }
        ...
        RightModules {
          id: bottomGroup
          anchors.bottom: parent.bottom
          anchors.bottomMargin: parent.islandGap + Style.space(8)
          anchors.horizontalCenter: parent.horizontalCenter
        }
      }
    }
  ...
  component CenterModules: Item {
    ...
    Component {
      ...
      Item {
        ...
        readonly property int islandHeight: Math.max(root.barSize - Style.space(4), 1)
        readonly property real contentWidth: centerRoot.hasAnchor
          ? (beforeList.width + centerAnchorModule.width + afterList.width)
          : singleList.width
        // The anchored layout hangs beforeList/afterList off the clock's own
        // edges, not off this Item's center, so their combined span is only
        // centered here when both sides happen to be equally wide. Wrap the
        // pill around their actual edges instead of the parent's midpoint.
        readonly property real islandX: centerRoot.hasAnchor
          ? (centerAnchorModule.x - beforeList.width - Style.space(16))
          : (singleList.x - Style.space(16))
        ...
        BorderSurface {
          visible: parent.contentWidth > 0
          x: parent.islandX
          anchors.verticalCenter: parent.verticalCenter
          width: parent.contentWidth + Style.space(32)
          height: parent.islandHeight
          radius: height / 2
          color: "transparent"
    ...
    Component {
      ...
      Item {
        ...
        readonly property int islandWidth: Math.max(root.barSize - Style.space(4), 1)
        readonly property real contentHeight: centerRoot.hasAnchor
          ? (beforeList.height + centerAnchorModule.height + afterList.height)
          : singleList.height
        // Mirrors islandX in horizontalCenterModules: wrap the pill around
        // beforeList/afterList's actual edges rather than the parent midpoint,
        // since the two sides aren't generally equal in height.
        readonly property real islandY: centerRoot.hasAnchor
          ? (centerAnchorModule.y - beforeList.height - Style.space(16))
          : (singleList.y - Style.space(16))
        ...
        BorderSurface {
          ...
          anchors.horizontalCenter: parent.horizontalCenter
          width: parent.islandWidth
          height: parent.contentHeight + Style.space(32)
          radius: width / 2
          color: "transparent"
          borderSpec: Border.flat(Qt.rgba(Color.accent.r, Color.accent.g, Color.accent.b, 0.8), 1)
        }
